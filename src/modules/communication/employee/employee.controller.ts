...
export class EmployeeController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve HR requests with optional filtering by status, priority, request type, date range, and search. Supports pagination.',
  })
  @ApiResponse({
    status: 200,
    description:
      'List of HR requests retrieved successfully. Returns paginated response when filters are applied.',
  ...
  @ApiOperation({
    summary: 'Get HR requests by employee',
    description: 'Returns all HR requests raised by a specific employee.',
  })
  @ApiQuery({
    name: 'employeeId',
    type: Number,
    required: true,
    description: 'Employee ID to fetch HR requests for.',
  })
  @ApiResponse({
    status: 200,
    description: 'Employee-specific HR requests retrieved successfully.',
  })
  async getMyHrRequests(
    @Query('employeeId', QueryParseIntPipe) employeeId: number,
  ) {
  ...
  @Departments('HR', 'Admin')
  @ApiOperation({
    summary: 'Get HR request by ID',
    description: 'Fetches a single HR request by its ID.',
  })
  ...
  @Departments('HR', 'Admin')
  @ApiOperation({
    summary: 'Get HR requests by priority',
    description: 'Retrieves HR requests filtered by priority.',
  })
  @ApiParam({
    name: 'priority',
    enum: RequestPriority,
    description: 'Priority level of HR requests.',
  })
  @ApiResponse({
    status: 200,
    description: 'Filtered HR requests retrieved successfully.',
  })
  ...
  @Departments('HR')
  @ApiOperation({
    summary: 'Get HR requests by status',
    description: 'Retrieves HR requests filtered by status (HR only).',
  })
  @ApiParam({
    name: 'status',
    enum: RequestStatus,
    description: 'Status of HR requests.',
  })
  @ApiResponse({
    status: 200,
    description: 'Filtered HR requests retrieved successfully.',
  ...
  @ApiOperation({
    summary: 'Create a new HR request',
    description: 'Allows employees to raise a new HR request.',
  })
  ...
  @Departments('HR', 'Admin')
  @ApiOperation({
    summary: 'Take HR action on a request',
    description: 'HR personnel can take an action on a specific HR request.',
  })
  @ApiParam({ name: 'id', type: Number, description: 'HR request ID' })
  @ApiQuery({
    name: 'hrEmployeeId',
    type: Number,
    required: true,
    description: 'ID of the HR employee taking action',
  })
  @ApiBody({ type: EmployeeHrActionDto })
  @ApiResponse({
    status: 200,
    description: 'HR action successfully taken on the request.',
  })
  async takeHrAction(
  ...
  @Departments('HR', 'Admin')
  @ApiOperation({
    summary: 'Update HR action on a request',
    description:
      'HR can update the status or details of an existing HR request.',
  })
  @ApiParam({ name: 'id', type: Number, description: 'HR request ID' })
  @ApiQuery({
    name: 'hrEmployeeId',
    type: Number,
    required: true,
    description: 'ID of the HR employee updating the request',
  })
  @ApiBody({ type: EmployeeHrActionDto })
  ...
  async updateHrRequestAction(
    ...
    if (req.user.type === 'admin') {
      return this.employeeService.updateAdminAction(
        id,
        hrActionDto,
        req.user.id,
      );
    ...
    if (!hrEmployeeId) {
      throw new BadRequestException(
        'HR Employee ID is required for updating actions',
      );
    }

    return this.employeeService.updateHrRequestAction(
      id,
      hrActionDto,
      hrEmployeeId,
    );
  ...
  @Departments('HR', 'Admin')
  @ApiOperation({
    summary: 'Delete an HR request',
    description: 'Allows HR to delete an existing HR request.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
    description: 'HR request ID to delete',
  })
  @ApiQuery({
    name: 'hrEmployeeId',
    type: Number,
    required: true,
    description: 'ID of the HR employee performing the deletion',
  })
  ...
  async deleteHrRequest(
    ...
    if (!hrEmployeeId) {
      throw new BadRequestException(
        'HR Employee ID is required for deleting requests',
      );
    }
