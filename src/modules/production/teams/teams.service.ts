...
export class TeamsService {
  ...
  async createTeam(createTeamDto: CreateProductionTeamDto, user?: any) {
    ...
    if (teamLeadEmployee.role.name !== 'team_lead') {
      throw new BadRequestException(
        'Employee must have team_lead role to be assigned as team lead',
      );
    }
    ...
    if (teamLeadEmployee.status !== 'active') {
      throw new BadRequestException(
        'Employee must be active to be assigned as team lead',
      );
    }
    ...
    if (!productionUnit) {
      throw new BadRequestException(
        `Production unit with ID ${productionUnitId} does not exist`,
      );
    }
    ...
    if (user && user.role === 'unit_head') {
      if (productionUnit.headId !== user.id) {
        throw new ForbiddenException(
          'You can only create teams in your own production unit',
        );
      }
    }
  ...
  async getAllTeams(user: any, query: TeamsQueryDto) {
    ...
    const teamsWithCounts = await Promise.all(
      teams.map(async (team) => {
        ...
        const membersCount = team.teamLeadId
  ...
  async getTeam(id: number, user?: any) {
    ...
    if (
      ...
      if (!belongsToTeam) {
        throw new ForbiddenException(
          'You do not have access to this team. You must be a member of this team to view its details.',
        );
      }
    }
  ...
  private async checkUserBelongsToTeam(
    user: any,
    teamId: number,
  ): Promise<boolean> {
  ...
  async updateTeam(id: number, updateTeamDto: UpdateTeamDto, user?: any) {
    ...
    if (user && user.role === 'unit_head') {
      ...
      if (!teamInUnit) {
        throw new ForbiddenException(
          'You can only update teams in your own production unit',
        );
      }
    }
    ...
    if (name === undefined && teamLeadId === undefined) {
      throw new BadRequestException(
        'At least one field (name or teamLeadId) must be provided for update',
      );
    }
    ...
    if (teamLeadId !== undefined) {
      if (teamLeadId === null) {
        ...
        if (employee.department.name !== 'Production') {
          throw new BadRequestException(
            'Employee must be in Production department to be assigned as team lead',
          );
        }
  ...
  async deleteTeam(id: number) {
    ...
    const members = existingTeam.teamLeadId
  ...
  async addMembersToTeam(
    ...
    if (!team.teamLeadId) {
      throw new BadRequestException(
        'Team must have a team lead before adding members',
      );
    }
    ...
    if (user && user.role === 'unit_head') {
      if (team.productionUnit && team.productionUnit.headId !== user.id) {
        throw new ForbiddenException(
          'You can only add members to teams in your own production unit',
        );
      }
    }
    ...
    if (employeesInTeams.length > 0) {
      const conflictDetails = employeesInTeams.map(
        (emp) =>
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const totalMembers = team.teamLeadId
  ...
  async removeMemberFromTeam(teamId: number, employeeId: number, user?: any) {
    ...
    if (user && user.role === 'unit_head') {
      ...
      if (!teamInUnit) {
        throw new ForbiddenException(
          'You can only remove members from teams in your own production unit',
        );
      }
    }
