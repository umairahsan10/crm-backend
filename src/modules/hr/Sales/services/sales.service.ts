...
export class SalesService {
  ...
  async getAllSalesDepartments() {
    try {
      ...
      this.logger.error(
        `Failed to retrieve sales departments: ${error.message}`,
      );
  ...
  async getSalesDepartmentById(id: number) {
    try {
      ...
      this.logger.error(
        `Failed to retrieve sales department ${id}: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to retrieve sales department: ${error.message}`,
      );
    }
  }

  async createSalesDepartment(
    dto: CreateSalesDepartmentDto,
    hrEmployeeId: number,
  ) {
    ...
    try {
      ...
      if (!employee) {
        throw new BadRequestException(
          `Employee with ID ${dto.employeeId} does not exist. Sales department records can only be created for existing employees.`,
        );
      }
      ...
      if (existingSalesDept) {
        throw new BadRequestException(
          `Sales department record already exists for employee ${dto.employeeId}. Use update endpoint instead.`,
        );
      }
      ...
      await this.createHrLog(
        hrEmployeeId,
        'sales_department_created',
        employee.id,
        logDescription,
      );

      this.logger.log(
        `Created sales department record for employee ${dto.employeeId}`,
      );
      return salesDepartment;
    } catch (error) {
      if (
      ...
      throw new BadRequestException(
        `Failed to create sales department: ${error.message}`,
      );
    }
  }
  ...
  async updateSalesDepartment(
    ...
    try {
      ...
      if (!employee) {
        throw new BadRequestException(
          `Employee with ID ${existingSalesDept.employeeId} no longer exists. Cannot update sales department record.`,
        );
      }
      ...
      if (
        dto.leadsClosed !== undefined &&
        dto.leadsClosed !== existingSalesDept.leadsClosed
      ) {
        changes.push(
          `Leads Closed: ${existingSalesDept.leadsClosed || 0} → ${dto.leadsClosed}`,
        );
      }
      if (
        dto.salesAmount !== undefined &&
        dto.salesAmount !== Number(existingSalesDept.salesAmount)
      ) {
        changes.push(
          `Sales Amount: ${existingSalesDept.salesAmount || 0} → ${dto.salesAmount}`,
        );
      }
      if (
        dto.salesUnitId !== undefined &&
        dto.salesUnitId !== existingSalesDept.salesUnitId
      ) {
      ...
      if (
        ...
        dto.commissionRate !== Number(existingSalesDept.commissionRate)
      ) {
        changes.push(
          `Commission Rate: ${existingSalesDept.commissionRate || 0}% → ${dto.commissionRate}%`,
        );
      }
      if (
        dto.commissionAmount !== undefined &&
        dto.commissionAmount !== Number(existingSalesDept.commissionAmount)
      ) {
        changes.push(
          `Commission Amount: ${existingSalesDept.commissionAmount || 0} → ${dto.commissionAmount}`,
        );
      }
      if (
        dto.salesBonus !== undefined &&
        dto.salesBonus !== Number(existingSalesDept.salesBonus)
      ) {
        changes.push(
          `Sales Bonus: ${existingSalesDept.salesBonus || 0} → ${dto.salesBonus}`,
        );
      }
      if (
        dto.withholdCommission !== undefined &&
        dto.withholdCommission !== Number(existingSalesDept.withholdCommission)
      ) {
        changes.push(
          `Withhold Commission: ${existingSalesDept.withholdCommission || 0} → ${dto.withholdCommission}`,
        );
      }
      if (
        dto.withholdFlag !== undefined &&
        dto.withholdFlag !== existingSalesDept.withholdFlag
      ) {
        changes.push(
          `Withhold Flag: ${existingSalesDept.withholdFlag ? 'Yes' : 'No'} → ${dto.withholdFlag ? 'Yes' : 'No'}`,
        );
      }
      if (
        dto.targetAmount !== undefined &&
        dto.targetAmount !== Number(existingSalesDept.targetAmount)
      ) {
        changes.push(
          `Target Amount: ${existingSalesDept.targetAmount || 0} → ${dto.targetAmount}`,
        );
      }
      if (
        dto.chargebackDeductions !== undefined &&
        dto.chargebackDeductions !==
          Number(existingSalesDept.chargebackDeductions)
      ) {
        changes.push(
          `Chargeback Deductions: ${existingSalesDept.chargebackDeductions || 0} → ${dto.chargebackDeductions}`,
        );
      }
      if (
        dto.refundDeductions !== undefined &&
        dto.refundDeductions !== Number(existingSalesDept.refundDeductions)
      ) {
        changes.push(
          `Refund Deductions: ${existingSalesDept.refundDeductions || 0} → ${dto.refundDeductions}`,
        );
      }
      ...
      this.logger.error(
        `Failed to update sales department ${id}: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to update sales department: ${error.message}`,
      );
    }
  }
  ...
  async deleteSalesDepartment(id: number, hrEmployeeId: number) {
    ...
    try {
      ...
      if (!employee) {
        throw new BadRequestException(
          `Employee with ID ${existingSalesDept.employeeId} no longer exists. Cannot delete sales department record.`,
        );
      }
      ...
      await this.createHrLog(
        hrEmployeeId,
        'sales_department_deleted',
        employee.id,
        logDescription,
      );

      this.logger.log(
        `Deleted sales department with ID ${id} for employee ${existingSalesDept.employeeId} and updated related records`,
      );
      return result;
    } catch (error) {
      if (
      ...
      this.logger.error(
        `Failed to delete sales department ${id}: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to delete sales department: ${error.message}`,
      );
    }
  }
  ...
  async updateCommissionRate(
    ...
    try {
      ...
      const existingSalesDepartment =
        await this.prisma.salesDepartment.findUnique({
          where: { id },
          include: {
            employee: {
              select: {
      ...
      if (!existingSalesDepartment) {
        throw new NotFoundException(
          `Sales department record with ID ${id} not found`,
        );
      }
      ...
      this.logger.log(
        `Commission rate updated for sales department ${id} to ${dto.commissionRate}%`,
      );
      return updatedSalesDepartment;
    } catch (error) {
      this.logger.error(
        `Failed to update commission rate for sales department ${id}: ${error.message}`,
      );
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new BadRequestException(
        `Failed to update commission rate: ${error.message}`,
      );
    }
  }
  ...
  async updateTargetAmount(
    ...
    try {
      ...
      this.logger.log(
        `Target amount updated for sales department ${id} to $${dto.targetAmount}`,
      );
      return updatedSalesDepartment;
    } catch (error) {
      this.logger.error(
        `Failed to update target amount for sales department ${id}: ${error.message}`,
      );
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new BadRequestException(
        `Failed to update target amount: ${error.message}`,
      );
    }
  }
}
