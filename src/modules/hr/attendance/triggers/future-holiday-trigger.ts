...
export class FutureHolidayTrigger {
  ...
  async checkAndMarkHolidayAttendance(): Promise<void> {
    try {
      ...
      this.logger.debug(
        `Checking for holidays on ${currentDate.toDateString()} at ${currentTime}`,
      );
      ...
      this.logger.log(
        `Holiday detected: ${todayHoliday.holidayName}. Checking for employees whose shifts are starting...`,
      );
      ...
      for (const employee of employeesToMark) {
        try {
          ...
          const checkinTime = this.createShiftDateTime(
            currentDate,
            employee.shiftStart!,
          );
          const checkoutTime = this.createShiftDateTime(
            currentDate,
            employee.shiftEnd!,
          );
                  ...
                  if (attendance) {
                    await tx.attendance.update({
                      ...
                      data: {
                        ...
                        absentDays:
                          attendance.absentDays && attendance.absentDays > 0
                            ? { decrement: 1 }
                            : undefined,
                      },
                    });
                  ...
                  if (summary) {
                    await tx.monthlyAttendanceSummary.update({
                      ...
                      data: {
                        ...
                        totalAbsent:
                          summary.totalAbsent && summary.totalAbsent > 0
                            ? { decrement: 1 }
                            : undefined,
                      },
                    });
      ...
      if (markedPresent > 0) {
        this.logger.log(
          `Holiday attendance marked: ${markedPresent} employees marked present, ${skipped} skipped, ${errors} errors for holiday: ${todayHoliday.holidayName}`,
        );
      }
    } catch (error) {
      ...
      if (
        ...
        this.logger.warn(
          `Database connection issue in future holiday trigger: ${error.message}`,
        );
      } else {
  ...
  private async updateAttendanceRecord(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update attendance record for employee ${employeeId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async updateMonthlyAttendanceSummary(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update monthly attendance summary for employee ${employeeId}: ${error.message}`,
      );
      throw error;
    }
  }
   ...
   */
  async manualTriggerForDate(
    holidayDate: string,
  ): Promise<{ marked_present: number; errors: number; message: string }> {
    try {
      ...
      this.logger.debug(
        `Parsed date: ${targetDate.toISOString()}, Original: ${holidayDate}`,
      );
      ...
      this.logger.debug(
        `Holiday query result: ${holiday ? 'Found' : 'Not found'}`,
      );
        ...
        try {
          ...
          if (existingLog) {
            ...
            await this.prisma.attendanceLog.update({
              ...
              data: {
                status: 'present',
                checkin: this.createShiftDateTime(
                  targetDate,
                  employee.shiftStart || '09:00',
                ),
                checkout: this.createShiftDateTime(
                  targetDate,
                  employee.shiftEnd || '17:00',
                ),
                mode: 'onsite',
              },
            });
          ...
          this.logger.error(
            `Failed to mark employee ${employee.id} as present: ${error.message}`,
          );
          errors++;
        }
      }
