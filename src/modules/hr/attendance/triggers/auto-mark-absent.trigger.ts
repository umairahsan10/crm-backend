...
export class AutoMarkAbsentTrigger {
  ...
  private getAttendanceDate(): string {
    ...
    let attendanceDate = new Date(pktTime);
  ...
  async autoMarkAbsent(): Promise<{ message: string; absent_marked: number; leave_applied: number } | null> {
    ...
    try {
      ...
      const dateExplanation = (currentHour < 9 || (currentHour === 9 && currentMinute === 0))
        ? `previous day (triggered at ${timeStr} - before 09:01)`
        : `current day (triggered at ${timeStr} - after 09:00)`;
      
      this.logger.log(`Starting auto-mark-absent check at ${timeStr} PKT for date: ${attendanceDate} (${dateExplanation})`);
      
      const absentResult = await this.attendanceService.autoMarkAbsent(attendanceDate);
      this.logger.log(
