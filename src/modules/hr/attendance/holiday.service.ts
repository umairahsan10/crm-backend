...
export class HolidayService {
    ...
    try {
      ...
      if (existingHoliday) {
        throw new ConflictException(
          `Holiday already exists on ${createHolidayDto.holidayDate}`,
        );
      }
      ...
      if (holidayDate <= today) {
        ...
        await this.adjustAttendanceForPastHoliday(
          holidayDate,
          userId,
          userType,
        );
      ...
      if (userType === 'hr') {
        await this.createHrLog(
          ...
          'holiday_created',
          null,
          `Holiday "${createHolidayDto.holidayName}" created for ${createHolidayDto.holidayDate}${holidayDate <= today ? ' (Past date - attendance adjusted)' : ''}`,
        );
      }
  ...
  async isHoliday(
    date: string,
  ): Promise<{ isHoliday: boolean; holiday?: HolidayResponseDto }> {
    try {
  ...
  async isHolidayDate(
    ...
    try {
      ...
      this.logger.error(
        `Failed to check holiday status for date ${date}: ${error.message}`,
      );
  ...
  async getHolidayStats(year?: number): Promise<{
    ...
    try {
      ...
      const holidaysByMonthArray = Array.from(monthCounts.entries()).map(
        ([month, count]) => ({
          month,
          count,
        }),
   ...
   */
  private async adjustAttendanceForPastHoliday(
    holidayDate: Date,
    userId: number,
    userType: string,
  ): Promise<void> {
    try {
      this.logger.log(
        `Adjusting attendance for past holiday on ${holidayDate.toDateString()}`,
      );
      ...
      this.logger.log(
        `Successfully adjusted attendance for ${adjustedCount} employees for holiday on ${holidayDate.toDateString()}`,
      );

      if (errors > 0) {
        this.logger.warn(
          `${errors} employees had errors during attendance adjustment`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to adjust attendance for past holiday: ${error.message}`,
      );
      throw new BadRequestException(
        'Failed to adjust attendance for past holiday',
      );
    }
  }
