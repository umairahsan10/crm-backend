...
export class HalfDayLogResponseDto {
  ...
  @ApiProperty({
    description:
      'Scheduled time the employee was expected to check in (HH:mm:ss)',
  })
  scheduled_time_in: string;

  @ApiProperty({
    description: 'Actual time the employee checked in (HH:mm:ss)',
  })
  ...
  @ApiProperty({
    type: () => Object,
    description:
      'Attendance table updates (included only when action_taken is "Completed")',
