...
export class SalaryDeductionDto {
  @ApiPropertyOptional({ description: 'Employee ID to filter deductions', example: 123 })
  @IsOptional()
  @IsNumber()
  employeeId?: number;

  @ApiPropertyOptional({ description: 'Month for which deductions are calculated in YYYY-MM format', example: '2025-10' })
  @IsOptional()
  @IsString()
  month?: string; // Format: YYYY-MM

  @ApiPropertyOptional({ description: 'Start date for custom date range filter', example: '2025-10-01' })
  @IsOptional()
  @IsDateString()
  startDate?: string;

  @ApiPropertyOptional({ description: 'End date for custom date range filter', example: '2025-10-31' })
  @IsOptional()
  @IsDateString()
  endDate?: string;
}
...
export class DeductionCalculationDto {
  ...
  @ApiProperty({ description: 'Month of calculation in YYYY-MM format', example: '2025-10' })
  month: string;
  ...
  @ApiProperty({ description: 'Late days counted in monthly calculations', example: 1 })
  ...
  @ApiProperty({ description: 'Final salary after all adjustments', example: 44500 })
  finalSalary: number; // Salary after deductions are subtracted
}

export class SalaryDeductionResponseDto {
  @ApiProperty({ type: [DeductionCalculationDto], description: 'List of deductions calculations per employee' })
