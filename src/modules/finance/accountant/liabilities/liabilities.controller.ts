...
export class LiabilitiesController {
  ...
  @ApiResponse({
    ...
    type: LiabilityCreateResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Invalid input',
    type: LiabilityErrorResponseDto,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'List of liabilities with transaction details',
    type: LiabilityListResponseDto,
  })
  @ApiQuery({
    name: 'isPaid',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    description: 'Filter by related vendor ID',
    example: 5,
  })
  @ApiQuery({
    name: 'category',
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Search term to filter liabilities by name or category',
  })
      ...
      isPaid:
        query.isPaid === 'true'
          ...
          : query.isPaid === 'false'
            ? false
            : undefined,
      relatedVendorId: query.relatedVendorId
        ? parseInt(query.relatedVendorId)
        : undefined,
  ...
  @ApiResponse({
    status: 200,
    description: 'Liability statistics retrieved successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Liability details with transaction and vendor information',
    type: LiabilityDetailResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 200,
    description: 'Updated liability details',
    type: LiabilityUpdateResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Invalid input or liability cannot be updated',
    type: LiabilityErrorResponseDto,
  })
  ...
  @ApiResponse({
    status: 200,
    description:
      'Liability, transaction, and expense details after marking as paid',
    type: LiabilityMarkPaidResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Cannot mark liability as paid',
    type: LiabilityErrorResponseDto,
  })
  ...
  async markLiabilityAsPaid(
    ...
    const currentUserId = req.user.id;
    return await this.liabilitiesService.markLiabilityAsPaid(
      dto,
      currentUserId,
    );
  }
