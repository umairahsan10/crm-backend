...
export class FinanceService {
  ...
  async calculateSalary(
    ...
    this.logger.log(
      `⏳ Starting salary calc for employee ${employeeId} (start=${startDate ?? 'N/A'}, end=${endDate ?? 'N/A'})`,
    );
    ...
    const logEntry = await this.createSalaryLog(
      employeeId,
      salaryResult,
      deductionResult,
    );
    ...
    this.logger.log(
      `✅ Salary calculation completed for employee ${employeeId} - log id ${logEntry.id}`,
    );
    return logEntry;
  }
  ...
  async calculateSalaryManual(employeeId: number, endDate?: string) {
    this.logger.log(
      `⏳ Starting smart manual salary calc for employee ${employeeId} (end=${endDate ?? 'current date'})`,
    );
    ...
    this.logger.log(
      `✅ Smart manual salary calculation completed for employee ${employeeId} - log id ${logEntry.id}`,
    );
    return logEntry;
  }
  ...
  private async calculateSalaryInternal(
    ...
    if (!employeeId || isNaN(employeeId)) {
      throw new BadRequestException(
        `Invalid employee ID in calculateSalaryInternal: ${employeeId}`,
      );
    }
  ...
  private async createSalaryLog(
    ...
    if (existingRecord) {
      ...
      this.logger.log(
        `📝 Salary log updated for employee ${employeeId} - Net Salary: ${salaryResult.netSalary}, Deductions: ${deductions}`,
      );
    } else {
      // Create new record
      ...
      this.logger.log(
        `📝 Salary log created for employee ${employeeId} - Net Salary: ${salaryResult.netSalary}, Deductions: ${deductions}`,
      );
    }
  ...
  public async calculateAllEmployees() {
    ...
    this.logger.log(
      `🔄 Auto calculation: Processing salary for ${activeEmployees.length} active employees only`,
    );
    ...
    for (const emp of activeEmployees) {
      try {
        ...
        if (emp.startDate) {
          ...
          if (
            employeeStartMonth === currentMonth &&
            employeeStartYear === currentYear
          ) {
            this.logger.log(
              `New employee ${emp.id} started this month - calculating from start date ${emp.startDate}`,
            );
            ...
            this.logger.log(
              `Regular employee ${emp.id} - calculating full month salary`,
            );
          ...
          this.logger.log(
            `Employee ${emp.id} has no start date - calculating full month salary`,
          );
          await this.calculateSalary(emp.id);
        }
      } catch (err) {
  ...
  public async calculateAllEmployeesDeductions(month?: string) {
    ...
    this.logger.log(
      `🔄 Deduction calculation: Processing deductions for ${activeEmployees.length} active employees for month ${calculationMonth}`,
    );
    ...
    for (const emp of activeEmployees) {
      try {
        ...
        if (!attendanceSummary) {
          this.logger.warn(
            `No attendance data found for employee ${emp.id} for month ${calculationMonth}`,
          );
          continue;
        }
        ...
        this.logger.log(
          `✅ Deductions calculated for employee ${emp.id}: ${deductionResult.totalDeduction} total deduction`,
        );
      } catch (err) {
        this.logger.error(
          `Failed to calculate deductions for employee ${emp.id}: ${err.message}`,
        );
      }
    }

    // Log summary
    const totalDeductions = deductionResults.reduce(
      (sum, result) => sum + result.totalDeduction,
      0,
    );
    const totalEmployees = deductionResults.length;

    this.logger.log(
      `📊 Deduction calculation summary: ${totalEmployees} employees processed, ${totalDeductions} total deductions`,
    );
  ...
  async assignCommission(projectId: number) {
    ...
    if (!salesDepartment) {
      throw new NotFoundException(
        `Employee not found with ID: ${crackedLead.closedBy}`,
      );
    }
  ...
  async transferCommission(
    ...
    if (transferAmount.greaterThan(sourceField)) {
      throw new Error(
        `Insufficient funds in ${sourceFieldName}. Available: ${sourceField.toFixed(2)}, Requested: ${transferAmount.toFixed(2)}`,
      );
    }
    ...
    return {
      status: 'success',
      message:
        direction === 'release' ? 'Commission released' : 'Commission withheld',
      employee_id: employeeId,
  ...
  public async getAllSalariesDisplay(month?: string) {
    ...
    const results = employees.map((employee) => {
      ...
      const finalSalary = Math.max(
        0,
        baseSalary + totalBonus + commission - totalDeductions,
      );
    ...
    // Calculate summary
    const totalBaseSalary = results.reduce(
    ...
    const totalEmployeeBonus = results.reduce(
      (sum, result) => sum + result.employeeBonus,
      0,
    );
    const totalSalesBonus = results.reduce(
      (sum, result) => sum + result.salesBonus,
      0,
    );
    const totalBonus = results.reduce(
      (sum, result) => sum + result.totalBonus,
      0,
    );
    const totalAttendanceDeductions = results.reduce(
      (sum, result) => sum + result.attendanceDeductions,
      0,
    );
    const totalChargebackDeductions = results.reduce(
      (sum, result) => sum + result.chargebackDeduction,
      0,
    );
    const totalRefundDeductions = results.reduce(
      (sum, result) => sum + result.refundDeduction,
      0,
    );
    const totalDeductions = results.reduce(
      (sum, result) => sum + result.deductions,
      0,
    );
    const totalFinalSalary = results.reduce(
      (sum, result) => sum + result.finalSalary,
      0,
    );
