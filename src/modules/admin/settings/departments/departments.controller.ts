...
export class DepartmentsController {
   ...
   */
  @Get()
  @ApiOperation({ summary: 'Get all departments', description: 'Retrieve all departments with pagination and search. Admin only.' })
   ...
   */
  @Delete(':id')
  @ApiOperation({ summary: 'Delete department', description: 'Delete a department by ID. Cannot delete if department has employees. Admin only.' })
  @ApiParam({ name: 'id', type: Number, example: 1, description: 'Department ID' })
  @ApiOkResponse({ description: 'Department deleted successfully.', schema: { example: { message: 'Department deleted successfully' } } })
  @ApiBadRequestResponse({ description: 'Bad Request: Department has employees or deletion failed.' })
  @ApiNotFoundResponse({ description: 'Department not found.' })
  @ApiUnauthorizedResponse({ description: 'Unauthorized: Invalid or missing token.' })
  @ApiForbiddenResponse({ description: 'Forbidden: Only admins can access this endpoint.' })
