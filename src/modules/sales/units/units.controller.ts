...
export class UnitsController {
  ...
  async createUnit(
    @Body() createUnitDto: CreateSalesUnitDto,
    @Query() query: any,
  ) {
  ...
  @Departments('Sales')
  @ApiOperation({
    summary: 'Get all sales units with advanced filtering and pagination',
  })
  @ApiQuery({
    name: 'unitId',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      'Include related data (comma-separated: employees,leads,teams,head)',
  })
  @ApiQuery({
    name: 'hasHead',
    required: false,
  ...
  @ApiQuery({
    name: 'hasLeads',
    required: false,
    description: 'Filter units that have leads',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of units with pagination and filtering',
  })
  ...
  @ApiQuery({
    name: 'assigned',
    required: false,
    description: 'Filter by assigned heads (true/false)',
  })
  ...
  async getAvailableUnitHeads(@Query('assigned') assigned?: string) {
    ...
    if (assigned !== undefined) {
      if (assigned === 'true') {
        ...
        throw new BadRequestException(
          'assigned parameter must be true or false',
        );
      }
    }
  ...
  @Departments('Sales')
  @ApiOperation({
    summary: 'Get available teams for assignment to sales units',
  })
  @ApiQuery({
    name: 'assigned',
    required: false,
    description: 'Filter by assigned teams (true/false)',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Team successfully assigned to unit',
  })
  @ApiResponse({
    status: 400,
    description: 'Team already assigned or validation error',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Team successfully removed from unit',
  })
  ...
  @Departments('Sales')
  @ApiOperation({
    summary: 'Get details of a specific unit by ID with comprehensive data',
  })
  @ApiParam({ name: 'id', type: Number, description: 'Unit ID' })
  @ApiResponse({
    status: 200,
    description:
      'Unit details with teams, employees, and leads based on user role',
  })
