...
export class LeadsService {
  ...
  async getMyLeads(query: any, userId: number, userRole: string) {
    ...
    console.log(
      '🔍 User sales department info for my leads:',
      JSON.stringify(userSalesDept, null, 2),
    );
    ...
    console.log(
      '🔍 After type filtering, WHERE clause:',
    ...
    console.log(
      '🔍 Final WHERE clause for my leads:',
      JSON.stringify(where, null, 2),
    );
  ...
  async findAll(query: any, userRole: string, userId: number) {
    ...
    const hierarchicalFilters = await this.getHierarchicalFilters(
      userRole,
      userId,
      userSalesDept,
    );
  ...
  async requestLeads(
    requestLeadsDto: RequestLeadsDto,
    userId: number,
    userRole: string,
  ) {
    ...
    if (includePushLeads) {
      ...
      if (
        ...
        !roleAccessMap[userRole].includes('push')
      ) {
        throw new BadRequestException(
          'Push leads option is only available for senior and above roles',
        );
      }
    }
    ...
    return await this.prisma.$transaction(async (prisma) => {
      ...
      const leadsToCirculate = allAssignedLeads.filter(
        (lead) =>
          !keptLeadIds.includes(lead.id) && lead.status === 'in_progress',
      );

      // Validate that leads to circulate have outcomes
      const invalidLeads = leadsToCirculate.filter(
        (lead) => lead.outcome === null,
      );
      ...
      if (includePushLeads) {
        ...
        if (allAvailableLeads.length < 10) {
          throw new NotFoundException(
            `Insufficient leads available. Need 10, only ${allAvailableLeads.length} available (${warmColdLeads.length} warm/cold + ${pushLeads.length} push).`,
          );
        }
        ...
        if (availableLeads.length < 10) {
          throw new NotFoundException(
            `Insufficient leads available. Need 10, only ${availableLeads.length} available.`,
          );
        }
      ...
      const warmColdCount = assignedLeads.filter((lead) =>
        ['warm', 'cold'].includes(lead.type),
      ).length;
      const pushCount = assignedLeads.filter(
        (lead) => lead.type === 'push',
      ).length;
  ...
  async update(id: number, updateLeadDto: UpdateLeadDto, userId: number) {
    ...
    return await this.prisma.$transaction(async (prisma) => {
      ...
      if (updateLeadDto.outcome) {
        ...
        if (
          updateLeadDto.outcome === 'interested' &&
          lead.status !== 'in_progress'
        ) {
          throw new BadRequestException(
            'Lead must be in progress before it can be marked as interested',
          );
        }
        ...
        if (updateLeadDto.outcome === 'denied') {
          console.log(
            '🔍 Lead denied - current failedCount:',
            lead.failedCount,
          );
          ...
          if (newFailedCount >= 4) {
            console.log(
              '🔍 Lead reached 4 denials - marking as failed and archiving',
            );
            ...
            console.log(
              '🔍 Lead denied but not yet at threshold - updating failed count only',
            );
      ...
      if (updateLeadDto.status) {
        ...
        if (updateLeadDto.status === 'cracked') {
          ...
          if (lead.outcome !== 'interested') {
            throw new BadRequestException(
              'Lead must have "interested" outcome before it can be marked as cracked',
            );
          }
          ...
          if (!lead.startedById || lead.startedById !== userId) {
            throw new BadRequestException(
              'Lead must be started by the current user before it can be marked as cracked',
            );
          }

          if (!lead.assignedToId || lead.assignedToId !== userId) {
            throw new BadRequestException(
              'Lead must be assigned to the current user before it can be marked as cracked',
            );
          }
          ...
          if (!updateLeadDto.industryId) {
            throw new BadRequestException(
              'industryId is required when marking lead as cracked',
            );
          }
          if (!updateLeadDto.totalAmount) {
            throw new BadRequestException(
              'totalAmount is required when marking lead as cracked',
            );
          }
          if (!userId) {
            throw new BadRequestException(
              'closedById (current user) is required when marking lead as cracked',
            );
          }
          ...
          if (!industry) {
            throw new BadRequestException(
              'Industry with the provided industryId does not exist',
            );
          }
          ...
          if (!salesDept?.commissionRate) {
            throw new BadRequestException(
              'Commission rate not found for the current user in sales department',
            );
          }
        ...
        if (updateLeadDto.status === 'completed') {
          ...
          if (!lead.crackedById) {
            throw new BadRequestException(
              'Lead must be cracked before it can be marked as completed',
            );
          }
          if (!lead.assignedToId) {
            throw new BadRequestException(
              'Lead must be assigned to an employee before it can be marked as completed',
            );
          }
          if (!lead.startedById) {
            throw new BadRequestException(
              'Lead must have a started by employee before it can be marked as completed',
            );
          }
      ...
      if (updateLeadDto.action === 'push') {
        if (!updateLeadDto.comment) {
          throw new BadRequestException(
            'Comment is required when pushing a lead',
          );
        }
  ...
  async getCrackedLeads(query: any, userRole: string, userId: number) {
    ...
    if (Object.keys(hierarchicalFilters).length > 0) {
      ...
      console.log(
        '🔍 Applied hierarchical filters to lead:',
        hierarchicalFilters,
      );
    }
    ...
    console.log('🔍 Sorting - By:', sortBy, 'Order:', sortOrder);
    console.log(
      '🔍 Final WHERE clause for cracked leads:',
      JSON.stringify(where, null, 2),
    );
  ...
  async getCompletedLeads(query: any, employeeId: number) {
    ...
    if (userRole === 'dep_manager' || userRole === 'unit_head') {
      ...
      console.log(
        '🔍 User is regular employee - showing only leads cracked by them (crackedById:',
        employeeId,
        ')',
      );
    }
    ...
    console.log('🔍 Sorting - By:', sortBy, 'Order:', sortOrder);
    console.log(
      '🔍 Final WHERE clause for completed leads:',
      JSON.stringify(where, null, 2),
    );
    ...
    if (userRole === 'unit_head') {
      if (crackedLead.lead.salesUnitId !== userSalesDept?.salesUnitId) {
        throw new ForbiddenException(
          'You can only access cracked leads from your own unit',
        );
      }
      ...
      if (!teamMemberIds.includes(crackedLead.lead.assignedToId || 0)) {
        throw new ForbiddenException(
          'You can only access cracked leads assigned to you or your team members',
        );
      }
    } else if (userRole === 'senior' || userRole === 'junior') {
      if (crackedLead.lead.assignedToId !== userId) {
        throw new ForbiddenException(
          'You can only access cracked leads assigned to you',
        );
      }
    }
  ...
  async getLeadStatistics(userRole: string, userUnitId?: number) {
    ...
    const conversionRate =
      totalLeads > 0
        ? ((completedLeads / totalLeads) * 100).toFixed(2)
        : '0.00';
  ...
  private async getUserSalesDepartment(userId: number) {
    ...
    if (userSalesDept) {
      ...
      console.log(
        '🔍   Teams in unit:',
        userSalesDept.salesUnit?.teams?.length || 0,
      );
      userSalesDept.salesUnit?.teams?.forEach((team, index) => {
        console.log(
          `🔍     Team ${index + 1}: ID=${team.id}, Lead=${team.teamLeadId}`,
        );
      });
    } else {
      console.log(
        '🔍 ❌ No sales department record found for user ID:',
        userId,
      );
    }
  ...
  private async getHierarchicalFilters(
    userRole: string,
    userId: number,
    userSalesDept: any,
  ) {
    ...
    switch (userRole) {
      ...
      case 'team_lead':
        ...
        console.log(
          '🔍   → Can see leads assigned to team members of user ID:',
          userId,
        );
        ...
        console.log(
          '🔍   → Team member details:',
          teamMembers.map((m) => `${m.id}: ${m.firstName} ${m.lastName}`),
        );
        ...
        if (teamMemberIds.length === 0) {
          console.log(
            '🔍   → No team members found - team lead can only see their own leads',
          );
          return {
            salesUnitId,
        ...
        console.log(
          '🔍   → Final member IDs (including team lead):',
          allMemberIds,
        );
  ...
  async getSalesUnitsForFilter(userRole?: string) {
    console.log(
      '🔍 Getting sales units for filter dropdown, userRole:',
      userRole,
    );
  ...
  async getEmployeesForFilter(salesUnitId?: number, userRole?: string) {
    console.log(
      '🔍 Getting employees for filter dropdown, salesUnitId:',
      salesUnitId,
      'userRole:',
      userRole,
    );
    ...
    if (userRole === 'marketing_manager') {
      ...
      console.log(
        '🔍 Marketing manager - showing Sales and Marketing employees',
      );
    } else {
    ...
    console.log('🔍 Employee breakdown:', {
      ...
      marketing: formattedEmployees.filter((e) => e.department === 'Marketing')
        .length,
    });
  ...
  async getArchivedLeads(query: any, userRole: string, userId: number) {
    ...
    const userSalesDept = await this.getUserSalesDepartment(userId);
    console.log(
      '🔍 User sales department info for archived leads:',
      JSON.stringify(userSalesDept, null, 2),
    );
    ...
    if (query.archivedFrom) {
      where.archivedOn = {
        ...where.archivedOn,
        gte: new Date(query.archivedFrom),
      };
    ...
    if (Object.keys(hierarchicalFilters).length > 0) {
      ...
      if (hierarchicalFilters.salesUnitId) {
        ...
        console.log(
          '🔍 Applied unit restriction for archived leads:',
          hierarchicalFilters.salesUnitId,
        );
      }
      if (hierarchicalFilters.assignedToId) {
        where.assignedTo = hierarchicalFilters.assignedToId;
        console.log(
          '🔍 Applied assignedTo restriction for archived leads:',
          hierarchicalFilters.assignedToId,
        );
      }
    }
    ...
    console.log('🔍 Sorting - By:', sortBy, 'Order:', sortOrder);
    console.log(
      '🔍 Final WHERE clause for archived leads:',
      JSON.stringify(where, null, 2),
    );
  ...
  async getArchivedLead(id: number, userRole: string, userId: number) {
    ...
    if (userRole === 'unit_head') {
      if (archivedLead.unitId !== userSalesDept?.salesUnitId) {
        throw new ForbiddenException(
          'You can only access archived leads from your own unit',
        );
      }
  ...
  async reverseArchivedLead(
    archivedLeadId: number,
    userId: number,
    userRole: string,
  ) {
    // Implementation placeholder for future reversibility feature
    throw new BadRequestException(
      'Lead reversibility feature is not yet implemented. This feature will allow reversing archived leads back to active leads.',
    );
  }
}
