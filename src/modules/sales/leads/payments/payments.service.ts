...
export class PaymentsService {
    ...
    try {
      ...
      if (crackedLead.closedBy !== userId) {
        throw new ForbiddenException(
          'Only the sales representative who cracked this lead can generate payment link',
        );
      }
      if (
        crackedLead.remainingAmount !== null &&
        dto.amount > crackedLead.remainingAmount
      ) {
        throw new BadRequestException(
          `Phase amount (${dto.amount}) cannot exceed remaining amount (${crackedLead.remainingAmount})`,
        );
      }

      // Check if this is the final phase (current phase equals total phases)
      if (
        crackedLead.currentPhase &&
        crackedLead.totalPhases &&
        ...
        if (
          ...
          throw new BadRequestException(
            `Final phase amount (${dto.amount}) must exactly match remaining amount (${crackedLead.remainingAmount})`,
          );
        }
      }
      ...
      const result = await this.prisma.$transaction(async (prisma) => {
        ...
        if (dto.clientId) {
          ...
          if (!client) {
            throw new BadRequestException(
              `Client with ID ${dto.clientId} not found. Please provide client information to create a new client.`,
            );
          }
  ...
  private async simulateSquareApiCall(
    dto: GeneratePaymentLinkDto,
  ): Promise<boolean> {
  ...
  async updatePaymentLinkDetails(
    ...
    if (transaction.employeeId !== userId) {
      throw new ForbiddenException(
        'Only the creator can update this payment record',
      );
    }
    ...
    const result = await this.prisma.$transaction(async (prisma) => {
      ...
      if (updateDto.amount !== undefined) {
        ...
        if (!transaction.invoice?.leadId) {
          throw new BadRequestException(
            'No associated lead found for this transaction',
          );
        }
        ...
        if (crackedLead && crackedLead.remainingAmount !== null) {
          if (crackedLead.remainingAmount <= 0) {
            throw new BadRequestException(
              'No remaining amount available for this lead',
            );
          }

          if (updateDto.amount > crackedLead.remainingAmount) {
            throw new BadRequestException(
              `Phase amount (${updateDto.amount}) cannot exceed remaining amount (${crackedLead.remainingAmount})`,
            );
          }
        }
    ...
    return {
      ...
      data: {
        ...
        updatedFields: {
          transaction: Object.keys(updateDto).filter((key) =>
            ['amount', 'type', 'method'].includes(key),
          ),
          client: Object.keys(updateDto).filter((key) =>
            [
              'clientName',
              'companyName',
              'email',
  ...
  async handlePaymentCompletion(
    ...
    try {
      ...
      if (transaction.employeeId !== userId) {
        throw new ForbiddenException(
          'Only the creator can complete this payment',
        );
      }
      ...
      await this.prisma.$transaction(async (prisma) => {
        ...
        if (crackedLead && crackedLead.remainingAmount !== null) {
          const newRemainingAmount = Math.max(
            0,
            crackedLead.remainingAmount - amount,
          );
      ...
      if (transaction.invoice?.leadId) {
        ...
        if (crackedLead && crackedLead.currentPhase === 1) {
          ...
          console.log(
            `Creating project for first phase payment - Transaction: ${transactionId}, CrackedLead: ${crackedLead.id}`,
          );
          try {
            const projectResult = await this.projectsService.createFromPayment(
              ...
              null,
            ); // null for internal call

            if (projectResult.success) {
              projectCreated = true;
              projectData = projectResult.data;
              console.log(
                `Project created successfully: ${projectResult.data.id}`,
              );
            } else {
            ...
            console.warn(
              `Project creation failed for transaction ${transactionId}: ${error.message}`,
            );
          }
        }
      }
      ...
      const revenueResult = await this.revenueService.createRevenue(
        revenueDto,
        userId,
      );
      ...
      if (revenueResult.status === 'error') {
        ...
        console.warn(
          `Revenue creation failed for transaction ${transactionId}: ${revenueResult.message}`,
        );
      }
      ...
      if (transaction.invoice?.leadId) {
        ...
        if (updatedCrackedLead) {
          ...
          if (project) {
            ...
            console.log(
              `Updated project ${project.id} - payment stage: ${paymentStage}, live progress: ${liveProgress}% (Phase: ${currentPhase}/${totalPhases}, Completed phases: ${completedPhases})`,
            );
          }
        }
      }
      ...
      if (transaction.invoice?.leadId) {
        ...
        if (updatedCrackedLead) {
          ...
          if (
            updatedCrackedLead.remainingAmount !== null &&
            updatedCrackedLead.remainingAmount > 0
          ) {
            const newCurrentPhase = (updatedCrackedLead.currentPhase || 0) + 1;

            // Check if current phase would exceed total phases
            if (
              updatedCrackedLead.totalPhases &&
              newCurrentPhase > updatedCrackedLead.totalPhases
            ) {
              throw new BadRequestException(
                `Current phase (${newCurrentPhase}) cannot exceed total phases (${updatedCrackedLead.totalPhases})`,
              );
            }
      ...
      return {
        success: true,
        message:
          'Payment completed, project created (if first phase), payment stage updated, client payment logged, and revenue generated successfully',
        data: {
          transactionId: transactionId,
          ...
          project: projectData,
          revenue:
            revenueResult.status === 'success'
              ? (revenueResult as any).data?.revenue
              : null,
