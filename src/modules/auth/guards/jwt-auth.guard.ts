...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  private get isDebugMode(): boolean {
    return this.jwtConfig.isDebugMode();
  }
  ...
  canActivate(context: ExecutionContext) {
    ...
    if (!authHeader.startsWith('Bearer ')) {
      if (this.isDebugMode) {
        console.log(
          '❌ JWT Guard - Authorization header does not start with "Bearer "',
        );
      }
    ...
    if (tokenParts.length !== 3) {
      if (this.isDebugMode) {
        console.log(
          '❌ JWT Guard - Token does not have 3 parts (malformed JWT)',
        );
        console.log('🔍 JWT Guard - Token parts count:', tokenParts.length);
        console.log(
          '🔍 JWT Guard - Token preview:',
          token.substring(0, 50) + '...',
        );
      }
      throw new UnauthorizedException(
        'Malformed JWT token. Expected format: header.payload.signature',
      );
    }
