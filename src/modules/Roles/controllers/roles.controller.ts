...
export class RolesController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard, DepartmentsGuard)
  @Departments('HR', 'Admin')
  @ApiOperation({ summary: 'Get all roles with optional filters and pagination' })
  @ApiQuery({ name: 'search', required: false, type: String })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  @ApiResponse({ status: 200, description: 'List of roles', type: RolesListResponseDto })
  ...
  @ApiParam({ name: 'id', type: Number, description: 'Role ID' })
  @ApiResponse({ status: 200, description: 'Role deleted successfully', schema: { example: { message: 'Role deleted successfully' } } })
